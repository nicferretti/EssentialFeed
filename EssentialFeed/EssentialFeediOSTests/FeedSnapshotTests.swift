...
final class FeedSnapshotTests: XCTestCase {
    ...
    private func assert(snapshot: UIImage, named name: String, file: StaticString = #file, line: UInt = #line) {
        ...
        if snapshotData != storedSnapshotData {
            ...
            XCTFail("New snapshot does not match stored snapsot. New snapshot URL: \(temporarySnapshotURL), Stored snapshot URL: \(snapshotURL)", file: file, line: line)
        }
    }
...
private class ImageStub: FeedImageCellControllerDelegate {
    ...
    func didRequestImage() {
        cellController?.display(ResourceLoadingViewModel(isLoading: false))

        if let image {
            cellController?.display(image)
            cellController?.display(ResourceErrorViewModel(message: .none))
        } else {
            cellController?.display(ResourceErrorViewModel(message: "any"))
        }
    }
